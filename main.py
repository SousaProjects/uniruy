...
def inserir():
    ...
    lista = [nome, modelo, fabricante, custo, quantidade, data]
    
    if nome == "" or modelo == "" or fabricante == "" or custo == "" or quantidade == "":
        messagebox.showerror('Erro', 'Verifique se todos os campos foram preenchidos')
        
    else:
        
        if custo.isdigit() and quantidade.isdigit():
            ...
            messagebox.showerror('Erro', 'Os campos de Custo/Quantidade devem conter somente números')
...
def atualizar_main():
    try:
        ...
        def atualizar():
            ...
            lista = [nome, modelo, fabricante, custo, quantidade, data, entry_id]
        ...
        button_confirmar.place(x=240, y=371)
...
def delete_item():
    try:
        ...
        treev_dicionario = tree.item(treev_dados)
        tree_lista = treev_dicionario["values"]
        valor_id = [tree_lista[0]] # Os dados precisam estar em formato de lista
...
button_limpar.place(x=11, y=371)
...
def mostrar():
    ...
    #cabeçalho da lista
    cabecalho = ['ID', 'Nome do Produto', 'Modelo', 'Fabricante', 'Custo', 'Qtd', 'Data']
    ...
    size_cabecalho_item = [30,170,80,160,50,60,110]
    na = 0
...
button_pesquisa = Button(frame_direita_cima, width=10, text="Pesquisar", command=mostrar_pesquisa)
button_pesquisa.place(x=520, y=12)

button_limpar_pesquisa = Button(frame_direita_cima, width=6, text="Limpar", command=limpar_pesquisa)
button_limpar_pesquisa.place(x=605, y=12)
